...
///
/// The [CustomCarouselView] present a scrollable list of items, each of which can dynamically
...
///
/// The [CustomCarouselView.itemExtent] property must be non-null and defines the base
...
///
/// {@tool dartpad}
/// Here is an example of [CustomCarouselView] to show the uncontained layout. Each carousel
...
class _CustomCarouselViewState extends State<CustomCarouselView> {
  ...
  @override
  void didUpdateWidget(covariant CustomCarouselView oldWidget) {
...
class _RenderSliverFixedExtentCarousel
  ...
  double _buildItemExtent(int index,
  ...
  @override
  double indexToLayoutOffset(@Deprecated(
      'The itemExtent is already available within the scope of this function. '
          'This feature was deprecated after v3.20.0-7.0.pre.') double itemExtent,
      int index,) {
    final int firstVisibleIndex =
    (constraints.scrollOffset / maxExtent).floor();
...
/// Scroll physics used by a [CustomCarouselView].
///
...
class CarouselScrollPhysics extends ScrollPhysics {
  /// Creates physics for a [CustomCarouselView].
  ...
  double _getTargetPixels(_CarouselPosition position,
      Tolerance tolerance,
      double velocity,) {
    double fraction;
...
/// Metrics for a [CustomCarouselView].
class _CarouselMetrics extends FixedScrollMetrics {
  /// Creates an immutable snapshot of values associated with a [CustomCarouselView].
  _CarouselMetrics({
...
/// A controller for [CustomCarouselView].
///
...
class CarouselController extends ScrollController {
  ...
  /// The item that expands to the maximum size when first creating the [CustomCarouselView].
  final int initialItem;

  _CustomCarouselViewState? _carouselState;

  // ignore: use_setters_to_change_properties
  void _attach(_CustomCarouselViewState anchor) {
    _carouselState = anchor;
  }

  void _detach(_CustomCarouselViewState anchor) {
